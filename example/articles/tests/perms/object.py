...
class ArticleUserObjectPermPermTestCase(ObjectPermTestCaseMixin, ArticleUserPermTestCase):
    ...
    def test_add_object_perm_by_str(self):
        ...
        self.user.fperms.add_perm('object.articles.Article.add', obj=self.article)

        self.assertTrue(self.user.fperms.has_perm(add_obj_perm))

    def test_fail_add_object_perm_non_existent_codename(self):
        self._create_perm()
        with self.assertRaises(Perm.DoesNotExist):
            self.user.fperms.add_perm('object.articles.Article.delete', obj=self.article)

    def test_fail_add_object_perm_non_existent_object(self):
        self._create_perm()
        with self.assertRaises(Perm.DoesNotExist):
            self.user.fperms.add_perm('object.articles.Article.add', obj=self.article2)

    def test_has_model_perm_from_wildcard(self):
        self._create_wildcard_perm()
        self.user.fperms.add_perm('object.articles.Article.*', obj=self.article)

        self.assertTrue(self.user.fperms.has_perm('object.articles.Article.whatever', self.article))
...
class ArticleGroupObjectPermPermTestCase(ObjectPermTestCaseMixin, ArticleGroupPermTestCase):
    ...
    def test_add_object_perm_by_str(self):
        ...
        self.group.fperms.add_perm('object.articles.Article.add', obj=self.article)

        self.assertTrue(self.group.fperms.has_perm(add_obj_perm))

        # test perm is correctly available to the user as well
        self.assertFalse(self.user.fperms.has_perm(add_obj_perm))
    ...
    def test_fail_add_object_perm_by_non_existent_codename(self):
        self._create_perm()
        with self.assertRaises(Perm.DoesNotExist):
            self.group.fperms.add_perm('object.articles.Article.delete', obj=self.article)

    def test_fail_add_object_perm_by_non_existent_object(self):
        self._create_perm()
        with self.assertRaises(Perm.DoesNotExist):
            self.group.fperms.add_perm('object.articles.Article.add', obj=self.article2)
